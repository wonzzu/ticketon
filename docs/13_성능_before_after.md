...
> ⚠️ 이 프로젝트는 **측정 환경을 의도적으로 만들어 둔** 단계다. N+1을 일부러 살려두고(코드 곳곳의 `// TODO N+1`), 캐시도 꺼둔([05 §7](05_공연_좌석.md)) 이유는 "before"를 직접 보기 위해서다.
...
3. **캐시 활성화**([05 §7](05_공연_좌석.md)의 주석 해제) — 읽기 압도적인 공연 목록/상세.
